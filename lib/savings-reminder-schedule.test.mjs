...
test('route claims and records the resolved scheduled identity', () => {
  ...
  assert.match(reminderRouteSource, /claimSavingsReminderDelivery\(/);
  assert.match(reminderRouteSource, /effectiveDate: scheduledIdentity\.date/);
  assert.match(reminderRouteSource, /scheduleSlot: scheduledIdentity\.slot/);
  assert.match(reminderRouteSource, /slot: scheduledIdentity\.slot,[\s\S]*?date: scheduledIdentity\.date/);
  assert.doesNotMatch(reminderRouteSource, /existingNotifications|filterUnsentReminderSlots/);
});
...
test('midnight reminder uses its scheduled date for suppression and expense context', () => {
  ...
  const canonicalClaimIndex = reminderRouteSource.indexOf(
    'claimSavingsReminderDelivery(',
  );

  assert.ok(resolvedSlotIndex >= 0);
  assert.ok(canonicalClaimIndex > resolvedSlotIndex);
  assert.doesNotMatch(reminderRouteSource, /goal\.last_deposit_date !== scheduledIdentity\.date/);
  assert.match(reminderRouteSource, /effectiveDate: scheduledIdentity\.date/);
  ...
  assert.doesNotMatch(reminderRouteSource, /action_type !== 'SKIP_SAVINGS'/);
});
