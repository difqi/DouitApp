...
test("transaction_kind metadata does not create a second amount or budget axis", () => {
  const rows = [
    transaction({ id: "ordinary", amount: 100_000, transactionKind: "ORDINARY" }),
    transaction({ id: "fee", amount: 20_000, transactionKind: "FEE" }),
    transaction({ id: "saving", amount: 30_000, transactionKind: "SAVING" }),
  ];
  const [summary] = aggregateApprovedTransactionsByParentCategory(rows, [food]);
  assert.equal(summary.expense, 150_000);
  assert.equal(summary.count, 3);
  assert.deepEqual(calculateParentBudgetUsage(summary.expense, summary.budget), {
    spent: 150_000,
    remaining: 350_000,
    usagePercentage: 30,
  });
