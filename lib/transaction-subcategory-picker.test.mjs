...
test("create and edit keep Nabung visible while an empty child list hides only the picker", () => {
  assert.doesNotMatch(createSource, /category\.name\s*!==\s*SYSTEM_CATEGORY_NAMES\.SAVING/);
  assert.doesNotMatch(workspaceSource, /c\.name\s*!==\s*SYSTEM_CATEGORY_NAMES\.SAVING/);
